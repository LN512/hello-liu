...
v1 = tf.Variable(tf.zeros([3,3,3]),name='v1')  # 生成0矩阵
v2 = tf.Variable(tf.ones([10,5]),name='v2')   # 生成1矩阵
v3 = tf.Variable(tf.fill([2,3],9))  # 填充单值矩阵
v4_1 = tf.constant([1,2,3,4,5,6,7])
v4_2 = tf.constant(-1.0,shape=[2,3]) # 常量矩阵
v6_1 = tf.linspace(10.0,20.0,30,name='linespace')
v7_1 = tf.range(10,20,3)    # 生成等差序列
...
v8_5 = tf.Variable(tf.random_shuffle([[1,2,3],[4,5,6],[6,6,6]],seed=134,name='v8_5'))
...
with tf.Session() as sess:
    ...
    # 保存图的变量
    # save_path = saver.save(sess,'E:\daima\python\tensorflow\model.ckpt')
    # 加载图的变量
